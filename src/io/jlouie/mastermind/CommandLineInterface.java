...
public class CommandLineInterface extends UserInterface {
	...
	public Code inputAnswer() {
		...
		} while (login.length != Main.POS_NUM);
		return toCode(login);
	}
...
//		System.out.println(viewPossible);
	...
	public void directions() {
		...
		System.out.println("Enter a " + Main.POS_NUM + " digit number with each digit ranging from 0 to " + (Main.COLOR_NUM - 1) + ".");
	}
	...
	public void menu() {
		...
		Main.COLOR_NUM = Integer.parseInt(c.readLine("Number of colors: "));
		Main.POS_NUM = Integer.parseInt(c.readLine("Number of positions: "));
